    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="page-subtitle">Поиск, карточки и контакты клиентов.</p>
        </div>
        <button
          type="button"
          onClick={openCreate}
        ...
        <div className={`grid gap-3 border-b border-slate-200 p-4 ${canSeeAll ? 'md:grid-cols-3' : 'md:grid-cols-2'}`}>
          <label className="text-sm font-medium text-slate-700">
            ...
            <input
              ...
              className="mt-1 w-full rounded-md border border-slate-300 px-4 py-2 outline-none focus:border-cyan-500"
            />
          </label>
          ...
          <table className="table-compact">
            <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
              <tr>
                ...
                <th className="px-3 py-2.5">Контакты</th>
                <th className="px-3 py-2.5">Компания</th>
                {canSeeAll && <th className="px-3 py-2.5">Владелец</th>}
            ...
            <tbody className="divide-y divide-slate-100">
              {clientsQuery.isLoading && (
                <tr>
                  <td colSpan={columns} className="px-3 py-6 text-center text-slate-500">
                    Загрузка клиентов...
                  </td>
                </tr>
              )}
              {clientsQuery.data?.items.map((client) => (
                <tr key={client.id}>
                  ...
                  <td className="px-3 py-2.5 text-slate-600">{client.company ?? '-'}</td>
                  {canSeeAll && (
                    <td className="px-3 py-2.5 text-slate-600">
                      {getUserLabel(usersQuery.data, client.owner_id)}
                    </td>
                  )}
                  <td className="px-3 py-2.5 text-slate-600">{formatDate(client.created_at)}</td>
                  <td className="px-3 py-2.5">
