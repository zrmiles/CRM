...
export function ClientDetail() {
  ...
  if (clientQuery.isLoading) {
    return <div className="app-panel rounded-lg p-5 text-slate-500">Загрузка клиента...</div>
  }

  if (!clientQuery.data) {
    return <div className="app-panel rounded-lg p-5 text-slate-500">Клиент не найден.</div>
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="page-subtitle">{client.company ?? 'Без компании'}</p>
        </div>
      ...
      <section className="grid gap-4 md:grid-cols-4">
        <article className="app-panel rounded-lg p-4">
          <p className="text-sm text-slate-500">Электронная почта</p>
          ...
          <div className="mt-4 space-y-3">
            ...
            {dealsQuery.data?.items.map((deal) => (
              <Link
                key={deal.id}
                to={`/deals/${deal.id}`}
                className="block rounded-md border border-slate-200 p-4 hover:border-cyan-300"
              >
