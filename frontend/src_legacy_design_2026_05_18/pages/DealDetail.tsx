...
export function DealDetail() {
  ...
  if (dealQuery.isLoading) {
    return <div className="rounded-2xl bg-white p-6 text-slate-500">Загрузка сделки...</div>
  }

  if (!dealQuery.data) {
    return <div className="rounded-2xl bg-white p-6 text-slate-500">Сделка не найдена.</div>
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-slate-950">{deal.title}</h1>
          <p className="mt-1 text-sm text-slate-500">{stage?.name ?? 'Стадия не найдена'}</p>
        </div>
